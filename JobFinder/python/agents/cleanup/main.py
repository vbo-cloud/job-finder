...
def _cleanup(session: Session) -> tuple[int, int, int]:
    ...
    Grace period: offer_fetch_scheduler triggers a full refresh 1×/day (18:00 Europe/Paris
    local time) — offer_fetching itself is purely event-driven and has no schedule of its
    own (see its module docstring). With CLEANUP_COLLECTED_AGE_DAYS=2 an offer must miss
    2 consecutive fetch cycles before being purged — enough to survive a transient fetching
    outage without wiping the offer pool.
