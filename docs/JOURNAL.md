...
**Backend — agent `cv_analysis` étendu :** un appel GPT-4o-mini supplémentaire à chaque upload, best-effort et isolé — un échec ne bloque jamais le pipeline ROME → matching. Gratuit, jamais gaté par des crédits. `AZURE_OPENAI_ROME_DEPLOYMENT` renommé `AZURE_OPENAI_CHAT_DEPLOYMENT` (le déploiement sert désormais les deux analyses).
