...
- `app/profile/_components/CommuneZonePicker.tsx` : carte Leaflet cadrée sur la France métropolitaine entière (`fitBounds` + zoom minimal verrouillé + `maxBounds`, contrôle +/− retiré, chrome Leaflet thémé), tuiles CARTO assorties au thème via le token `--map-tiles`, bouton unique « Réinitialiser la zone », compteur de sélection. Composant contrôlé (`value`/`onChange`), importé via `dynamic(..., { ssr: false })`.
- `app/profile/_components/CommunePaintLayer.tsx` : charge la géométrie des 35 116 communes dès le montage (~8 Mo de GeoJSON statique) — la peinture fonctionne partout et à tout niveau de zoom ; pinceau circulaire de taille écran fixe — clic gauche peint, clic droit efface, molette zoome vers le curseur, glisser-molette déplace la carte ; test cercle/commune préfiltré par bbox ; la sélection est rendue par une couche canvas toujours visible, les contours non sélectionnés ne sont dessinés qu'à partir du zoom 7 (performance).
...
- **Garde sur les contrôles Leaflet** : les `mousedown` provenant de `.leaflet-control-container` (attribution) sont ignorés, sinon cliquer dessus peignait les communes situées dessous.
