...
- **`.github/CLAUDE_ACTION.md`** : documente le choix du déclenchement manuel-only (une piste de correction automatique frontend/backend basée sur le verdict `REQUEST_CHANGES` du bot de review existant a été envisagée puis abandonnée avant implémentation — voir Décisions techniques), le setup manuel requis (secret `ANTHROPIC_API_KEY`, installation de la GitHub App Claude), et les points de vérification encore ouverts.
