...
- **`doc-writer` avant `gh pr create`, hook-enforced ; avant les reviewers, instruction seule** : contrairement aux reviewer-*, `require_reviewer.py` (`Stop`) ne s'applique pas à `doc-writer` — l'exiger *avant* le passage des reviewers de façon mécanique aurait demandé de séquencer deux subagents déclenchés indépendamment l'un par rapport à l'autre (un hook `Stop` pour la fin de session, un hook `PreToolUse` pour `gh pr create`), ce qui ajoutait de la fragilité pour un bénéfice marginal. Choix : `pre_bash_guard.py` garantit seulement que `doc-writer` a tourné depuis la dernière édition avant que `gh pr create` ne soit autorisé ; l'ordre « avant les reviewers » reste une instruction dans la description du subagent lui-même, au même titre que la frontière Cowork/Code déjà documentée comme non hook-enforced plus haut dans `CLAUDE.md`.
